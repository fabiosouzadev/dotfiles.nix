{pkgs, ...}: {
  ...
  programs.zsh = {
    shellAliases = {
      ...
      aws_zapay_baixar_imagem_api = "aws ecr get-login-password --region sa-east-1 --profile aws-zapay-prod | docker login --username AWS --password-stdin 071032557399.dkr.ecr.sa-east-1.amazonaws.com";
    };
  };
}
